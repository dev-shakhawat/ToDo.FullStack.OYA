...
async function deleteTodo(req , res){
    ...
    try{
        ...
        if(todo.media || todo.mediaPublicID) await deleteImage(todo.mediaPublicID , todo.mediaType)

        await todoSchema.findByIdAndDelete(id)  // delete todo in database

        return res.status(200).send({ success: true , message : "Deleted successfully" , id })  // send success message to client
